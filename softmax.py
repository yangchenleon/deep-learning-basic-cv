...
if __name__ == '__main__':
    ...
    torch.save(net.state_dict(), './models/softmax.pth')

    # predict
    model = net
    model.load_state_dict(torch.load('./models/softmax.pth', map_location=device))
    X, y = next(iter(DataLoader(mnist_test, batch_size=18, shuffle=True)))  # X, y = iter(test_iter).next()
